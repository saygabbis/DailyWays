...
### Whiteboard (Spaces) — tabelas e Storage

Para o canvas colaborativo nos **Spaces**:

1. **Migration**: execute no SQL Editor (após as migrations iniciais e de groups/spaces):
   - `supabase/migrations/20260307000000_whiteboard_tables.sql` (cria `space_nodes`, `space_connectors`, `space_comments`, `space_assets` e adiciona as tabelas ao Realtime).

2. **Storage (imagens no canvas)**:
   - Em **Storage** no painel Supabase, crie um bucket chamado **`space-assets`**.
   - Marque como **Public** se quiser que as imagens tenham URL pública (recomendado para exibir no canvas).
   - Em **Policies** do bucket, adicione uma política de **INSERT** e **SELECT** para usuários autenticados (ex.: `auth.role() = 'authenticated'`), e **SELECT** público se o bucket for público.
